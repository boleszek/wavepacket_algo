function [wavepackets] = findwavepackets_hilb(x,params)
%% Use hilbert amplitude for detecting wps

% This modification uses amplitude of hilbert transorm envelope istead of
% wavelet energy profie for detecting wave packets.

% It also adds the calculation of empirical freuqency of each
% wavepacket, which is just the number of peaks divided by the length of
% the wavepacket (in seconds)

% Empirical fq calculated by using findpeaks function to find peaks, with a
% MinPeakProminence set to max(wp)/8 to avoid spurious small amplitude
% peaks

    % WARNING! for findpeaks to work you have to remove chronux from path! They
    % happen to have a function with the same name as the built-in matlab
    % function!

% Replaced all instances of splineEnergyProfile with energyProfile (no
% spline for Hilbert)

% Note: drawwavepackets option does not work with thes modifications

% Boleslaw Osinski 2017
...
% wave packet, something that is discrete. The packet is discrete. Yeah
% 
...
wavepackets.empfreq = 0; % empirical wavelet frequency
...
% Added by Bo
% Now apply the greedy algorithm on energyProfile (no need for spline with Hilbert)    
pk = zeros(size(energyProfile));
tmpEnergyProfile=energyProfile; %duplicate energy profile to use within inner loop        
chckdpts = ones(size(x));%vector for checked points
...
while true %run until no more significant peaks
	...
	tmpEnergyProfile(pk~=0)=0;

	% find biggest (current) peak
	[maxPeakVal,maxPeakLoc]= max(tmpEnergyProfile);
	...
	switch EXITCONDITION
		case 1
			snr = THRES*rms(energyProfile(pk<1)); %pk==0
	...
	while left>2 && left<(numel(x)-LAG) 
		if all( energyProfile(left-1) > energyProfile(left:(left+LAG)) )

			if energyProfile(left-1) > energyProfile(maxPeakLoc)/2 		
	...
	while (right-LAG>0) && right<(numel(x)-LAG) 
		if all( energyProfile(right+1) > energyProfile( (right-LAG):right ) )

		%still greater than half-energy. likely still part of same oscillation
			if energyProfile(right+1) < energyProfile(maxPeakLoc)/2 
			    break;
			end
		end
		%collision detection
...
%%%%%%%%%%% commented out by Bo %%%%%%%%%%%
        ...
        [pks] = findpeaks(x(left:right),left:right,'MinPeakProminence',max(x(left:right))/8);
        wavepackets.empfreq(ipeaks) = length(pks)/(wavepackets.length(ipeaks)/SF);
...
if DRAWWAVEPACKETS
	stairs(max(energyProfile)*logical(pk>0),'k')
	xlim([0 numel(x)])
end
